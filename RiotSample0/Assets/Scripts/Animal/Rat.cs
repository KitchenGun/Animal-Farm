...
public class Rat : Animal
{
    ...
    {//소 상태의 코루틴
        ...
        {//죽을때 까지 계속 
            HPCheck();//체력체크
            MoveContact();//충돌처리
            RatAnimator.SetBool("isMove", isMove);//이동애니메이션 체크
            switch (thisAnimalState)
            {
                case AnimalState.Idle://대기
                    break;
                ...
                case AnimalState.Attack://공격
                    ...
                    RatAnimator.SetBool("isAtk", false);
    ...
    private void Idle()
    {
        RatAnimator.Rebind();//모든 애니메이션 다시 시작 내부 변수값 초기화
    ...
    {//공격
        ...
        else
        {
            ...
            thisAnimalState = AnimalState.Attack;
            RatAnimator.SetBool("isAtk", true);
        }
    }
    #endregion
    ...
    {//후퇴버튼 클릭시 실행 함수
        ...
        RatAnimator.SetTrigger("isRetreat");
    }
    #endregion
    ...
    {//사망시 적용
        if (isDie == false)
        {
            ...
            RatAnimator.SetBool("isAtk", false);//애니메이션 제어
            RatAnimator.SetBool("isDie", true);
