...
public class Sheep : Animal
{
    ...
    {//소 상태의 코루틴
        ...
        {//죽을때 까지 계속 
            HPCheck();//체력체크
            MoveContact();//충돌처리
            SheepAnimator.SetBool("isMove", isMove);//이동애니메이션 체크
            switch (thisAnimalState)
            {
                case AnimalState.Idle://대기
                    break;
    ...
    private void Idle()
    {
        SheepAnimator.Rebind();//모든 애니메이션 다시 시작 내부 변수값 초기화
    ...
    {//공격
        ...
        else
        {
            ...
            thisAnimalState = AnimalState.Attack;
            SheepAnimator.SetBool("isAtk", true);
        }
    }
    #endregion
    ...
    {//후퇴버튼 클릭시 실행 함수
        ...
        SheepAnimator.SetTrigger("isRetreat");
    }
    #endregion
